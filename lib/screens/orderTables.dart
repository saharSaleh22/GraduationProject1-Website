...
String pathdelete="deleteOrder";
String choosenText=" Food Orders:";
...
int m=0;
  _initData() async {
    ...
    var response = await http.get(
      Uri.parse("http://192.168.1.35:3002/orders"),
      headers: _setHeader(),
    );
   print("my path : ");
  //  if(path=="recommended"){
  ...
  //  }
   print("deleete"+pathdelete);
    ...
    for (var data in valueMap) {
      ...
      myAllDaea.add(new ordersModel(data['name'],
          data['price'], 
          data['user'], data['quantity'],));
...
m= myAllDaea.length;
    ...
    myAllDaea.forEach((element) {
      print(
           '  phone :${element.name}    firstname :${element.price} ');
    });
    // setState(() {});
  }
  ...
  Widget build(BuildContext context) {
        ...
        _buildCard(int index){
           ...
           DataTable(
                        ...
                        rows: [ DataRow(cells: [
                            ...
                            DataCell(Text(myAllDaea[index].quantity.toString(),textAlign: TextAlign.center)),
    ...
    var response = await http.post(
      Uri.parse("http://192.168.1.35:3002/orderDelete"),
      body: jsonEncode({
        "name": name1,
      }),
      headers: _setHeader(),
